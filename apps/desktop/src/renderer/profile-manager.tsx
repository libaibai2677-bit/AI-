...
export function ProfileManager({ profiles, onProfilesChanged, onOpen }: Props) {
  ...
  const [translation, setTranslation] = useState<'DeepL' | 'Google'>('DeepL')
  ...
  const addProfile = async () => {
    ...
    setSaving(true)
    try {
      const created = await window.unifiedChat?.createProfile({ name: trimmed, provider, translation, language })
