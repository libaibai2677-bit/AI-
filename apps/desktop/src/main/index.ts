...
import { applyProviderSnapshotPersisted, clearPersistedProfileMessages, loadMessagesForProfile, loadUnifiedInbox, loadUnifiedMessageState, updateMessageTranslation } from './message-store'
...
function createWindow() {
  mainWindow = new BrowserWindow({ width: 1440, height: 900, minWidth: 980, minHeight: 640, backgroundColor: '#0b0d10', webPreferences: { preload: path.join(__dirname, '../preload/index.mjs'), contextIsolation: true, nodeIntegration: false } })
...
app.whenReady().then(async () => {
  ...
  if (!trustedUnlocked && vaultStatus.configured) mainWindow?.webContents.once('did-finish-load', () => mainWindow?.webContents.send('profile:vault-lock-required'))

  globalShortcut.register('CommandOrControl+Shift+L', async () => {
    if (await lockVault()) { lockProfileWindows(); providerSyncLoop?.stop(); mainWindow?.webContents.send('profile:quick-lock') }
  })
  for (let i = 1; i <= 9; i += 1) globalShortcut.register(`CommandOrControl+${i}`, () => mainWindow?.webContents.send('profile:quick-switch', i - 1))
  ...
  ipcMain.handle('profiles:set-health', (_event, profileId: string, component, status) => setProfileHealth(profileId, component, status))
  ipcMain.handle('profiles:set-status', (_event, profileId: string, status) => setProfileStatus(profileId, status))
  ipcMain.handle('profiles:set-window-state', (_event, profileId: string, bounds) => setProfileWindowState(profileId, bounds))
  ipcMain.handle('profiles:open', async (_event, profileId: string) => { const profiles = await loadProfiles(); const profile = profiles.find((item) => item.id === profileId); if (!profile) throw new Error('Profile not found'); await setActiveProfileId(profileId); openProfileWindow(profile); return profile })
  ipcMain.handle('profiles:backup', async (_event, profileId: string) => {
    const profiles = await loadProfiles(); const profile = profiles.find((item) => item.id === profileId); if (!profile) throw new Error('Profile not found')
    ...
    const { writeFile } = await import('node:fs/promises'); const payload: ProfileBackup = { format: 'unified-chat-profile', version: 1, exportedAt: new Date().toISOString(), profile: { id: profile.id, name: profile.name, provider: profile.provider, translation: profile.translation, language: profile.language, lastConversationId: profile.lastConversationId, windowState: profile.windowState } }; await writeFile(result.filePath, JSON.stringify(payload, null, 2), 'utf8'); return { canceled: false, filePath: result.filePath }
  })
  ipcMain.handle('profiles:restore', async () => { const result = await dialog.showOpenDialog(mainWindow ?? undefined, { title: 'Restore Profile Configuration', properties: ['openFile'], filters: [{ name: 'Unified Chat Profile', extensions: ['profile.json', 'json'] }] }); if (result.canceled || !result.filePaths[0]) return { canceled: true }; const { readFile } = await import('node:fs/promises'); const backup = JSON.parse(await readFile(result.filePaths[0], 'utf8')) as ProfileBackup; return { canceled: false, profile: await restoreProfileConfiguration(backup) } })
  ...
  ipcMain.handle('profiles:vault:unlock', async (_event, password?: string) => { const success = password ? await unlockVault(password) : await unlockFromTrustedDevice(); if (success) { unlockProfileWindows(); providerSyncLoop?.start() }; return success })
  ipcMain.handle('profiles:vault:lock', async () => { const success = await lockVault(); if (success) { lockProfileWindows(); providerSyncLoop?.stop() }; return success })
  ...
  ipcMain.handle('conversation-profile:set', (_event, profile) => setConversationProfile(profile))
  ...
  ipcMain.handle('messages:update-translation', (_event, profileId: string, messageId: string, translatedText: string) => updateMessageTranslation(profileId, messageId, translatedText))
...
app.on('will-quit', () => { providerSyncLoop?.stop(); globalShortcut.unregisterAll() })